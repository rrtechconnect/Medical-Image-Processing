...
blurredImage = real(ifft2(ifftshift(result)));
peaksnr0= PSNR(blurredImage, image);  
fprintf('\n The Peak-SNR value is %0.4f', peaksnr0);
subplot(5,4,2);
imshow(blurredImage); 
title(sprintf( 'Atm. Turbulence noise:PSNR %0.4f',peaksnr0 ));
%% Kernels Definition
...
% %% Convolution Process
% g9 = imfilter(blurredImage, w9, 'symmetric', 'conv');
% 
% %% Display the results
% figure, imshow(g9); title('Deblurred Image')
% % imwrite(g9,'out_Kernel.jpg')
% peaksnr1 = PSNR(blurredImage, image);  
% fprintf('\n The Peak-SNR value is %0.4f', peaksnr1);
% % fprintf('\n The SNR value is %0.4f \n', snr);
% peaksnr2 = PSNR(g9, image);  
